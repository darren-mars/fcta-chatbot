...
import { formatSelectionsForQuery } from '@/utils/formatSelections';
...
const basePromptPath = path.join(process.cwd(), 'src', 'config', 'systemPrompt.txt');
let basePrompt = '';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!vectorSearchResponse.ok) {
      throw new Error(`Vector search failed: Databricks token: ${config.databricksApiToken} ${await vectorSearchResponse.text()}`);
    }

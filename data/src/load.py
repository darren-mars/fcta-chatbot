...
from data.src.chunk import chunk_text_general, extract_pdf_text
...
def process_pdf_mount_point(
    ...
    def foreach_batch_pdf(df: DataFrame, batch_id: int):
        ...
        for row in df.collect():
            path_val = row["path"]
            pdf_chunks = extract_pdf_text(row["content"])  # list of chunk strings
        ...
        if not rows_for_df:
            logger.info("No PDF chunks in this micro-batch, skipping creation.")
            return
        ...
        pdf_df.write \
            .format("delta") \
            .mode("append") \
            .option("checkpointLocation", f"{pdf_checkpoint_path}/batch_{batch_id}") \
            .saveAsTable(f"{config['delta_table']['catalog_name']}.{config['delta_table']['schema_name']}.{config['delta_table']['table_name']}")


    pdf_query = (
        pdf_stream_df.writeStream
        .foreachBatch(foreach_batch_pdf)
...
def process_csv_mount_point(
    ...
    - If subdir_name == "packages-website", we:
      ...
      2) If length > 2,000, chunk only 'product_description' while repeating other columns as metadata.
    ...
    for subdir in mount.get("subdirectories", []):
        ...
        logger.info(f"Processing CSV subdir '{subdir_name}' with schema '{schema_name}'.")
        ...
        if subdir_name == "packages-website":
            ...
            def foreach_batch_csv_packages(df: DataFrame, batch_id: int):
                ...
                def split_text_if_needed(desc: str, threshold: int = 2000) -> List[str]:
                    ...
                    return [desc[i : i + threshold] for i in range(0, len(desc), threshold)]
                ...
                if not final_rows:
                    logger.info("No CSV rows found in this micro-batch after processing.")
                    return
                ...
                chunk_df.write \
                    .format("delta") \
                    .mode("append") \
                    .option("checkpointLocation", f"{csv_checkpoint_path}/batch_{batch_id}") \
                    .saveAsTable(f"{config['delta_table']['catalog_name']}.{config['delta_table']['schema_name']}.{config['delta_table']['table_name']}")

            csv_query = (
                package_df.writeStream
                .foreachBatch(foreach_batch_csv_packages)
                .option("checkpointLocation", csv_checkpoint_path)
                .trigger(availableNow=True)
                .start()
            )
            csv_query.awaitTermination()
            logger.info(f"Completed CSV ingestion for '{subdir_name}' with schema '{schema_name}'.")
            ...
            def foreach_batch_csv_other(df: DataFrame, batch_id: int):
                logger.info(f"[CSV - {subdir_name}] Micro-batch {batch_id}, {df.count()} records.")
                collected_rows = df.collect()
            ...
            csv_query = (
                contents_df.writeStream
                .foreachBatch(foreach_batch_csv_other)
                .option("checkpointLocation", csv_checkpoint_path)
                .trigger(availableNow=True)
                .start()
            )
...
def write_to_delta_table(unified_df: DataFrame, table_config: Dict[str, Any], checkpoint_path: str):
    ...
    try:
        ...
        logger.error(f"Error writing to Delta table '{full_table_name}': {e}", exc_info=True)
        raise
        ...
        for mount in config.get("mount_points", []):
            ...
            if data_source_type == "pdf":
                pdf_df = process_pdf_mount_point(
                    spark, mount, config, include_existing_files="true" if initial_run else "false"
                )
                ...
                logger.warning(f"Unknown data_source_type '{data_source_type}' at '{mount_point}'. Skipping.")

        unified_df = union_dataframes(df_list)
        if unified_df is not None:
            logger.info("Note: PDF/CSV data was written in 'foreachBatch'. The 'unified_df' is optional.")
